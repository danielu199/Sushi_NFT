...
# get a bot!
bot = InstaPy(username='kryptosushi', password='danirocks!', headless_browser=False)
...
with smart_run(bot):
    ...
    # general settings
    bot.set_dont_like(['sad', 'rain', 'depression'])
    bot.set_do_follow(enabled=True, percentage=80)
    ...
    bot.set_comments([
                         ...
                         media='Photo')
    bot.set_do_like(True, percentage=70)
    bot.set_delimit_liking(enabled=True, max_likes=100, min_likes=0)
    bot.set_delimit_commenting(enabled=True, max_comments=20, min_comments=0)
    ...
    bot.set_user_interact(amount=10, randomize=True, percentage=80)

    # activity
    bot.like_by_tags(my_hashtags, amount=90, media=None)
    bot.unfollow_users(amount=500, instapy_followed_enabled=True, instapy_followed_param="nonfollowers",
                           style="FIFO",
